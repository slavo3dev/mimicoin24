...
*Setup instructions for a Gitian build of Mimicoin24 Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Mimicoin24
...
- [Building Mimicoin24 Core](#building-mimicoin24-core)
...
Clone the git repositories for mimicoin24 and Gitian.
...
in the mimicoin24 repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Mimicoin24 Core
----------------

To build Mimicoin24 Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the mimicoin24 repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/mimicoin24/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/mimicoin24-project/mimicoin24
...
For example:
```bash
URL=https://github.com/thrasher-/mimicoin24.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit mimicoin24=${COMMIT} --url mimicoin24=${URL} ../mimicoin24/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit mimicoin24=${COMMIT} --url mimicoin24=${URL} ../mimicoin24/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit mimicoin24=${COMMIT} --url mimicoin24=${URL} ../mimicoin24/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the mimicoin24 git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../mimicoin24/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/mimicoin24
SIGPATH=/some/root/path/mimicoin24-detached-sigs

./bin/gbuild --url mimicoin24=${BTCPATH},signature=${SIGPATH} ../mimicoin24/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mimicoin24-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mimicoin24-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/mimicoin24-osx-build.assert
```
...
[mimicoin24-project/gitian.sigs.ltc](https://github.com/mimicoin24-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
