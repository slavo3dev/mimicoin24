Mimicoin24 Core version 0.13.2 is now available from:

  <https://download.mimicoin24.org/mimicoin24-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/mimicoin24-project/mimicoin24/issues>
...
updates will be released anymore. Without security updates, using a mimicoin24
...
ECDSA signatures inside Mimicoin24 transactions now use validation using
...
Previous versions of Mimicoin24 Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Mimicoin24 Core was to raise the default minimum
relay fee.

Mimicoin24 Core 0.13.2 will have a strict maximum size on the mempool. The
...
Mimicoin24 Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running mimicoin24d without having to do any manual configuration.
...
Mimicoin24 Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Mimicoin24 Core will relay transactions with
...
entirely in Mimicoin24 Core 0.13, and it is at this time undecided whether the
...
Mimicoin24 Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Mimicoin24 Core automatically creates a hidden service to listen on, without
manual configuration. Mimicoin24 Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Mimicoin24 Core is listening, and
...
Mimicoin24d can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Mimicoin24
Core to use floating fees. By default, Mimicoin24 Core will use floating
fees.
...
At all times, Mimicoin24 Core will cap fees at `-maxtxfee=<x>` (default:
0.10) LTC.
Furthermore, Mimicoin24 Core will never create transactions paying less than
...
`mimicoin24.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /mimicoin24rpc>
        ...
        # optional bypass mimicoin24d rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< mimicoin24rpc:<password>
    </Location>
...
Restarting mimicoin24d will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in mimicoin24.conf
...
mimicoin24-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/mimicoin24-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Mimicoin24 Core code base has
...
- `mimicoin24-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `mimicoin24-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Mimicoin24 scripting system that in combination with
...
Mimicoin24 transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Mimicoin24 Core.
...
The two phases are distinct in the Mimicoin24-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Mimicoin24 wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Mimicoin24.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Mimicoin24 block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of mimicoin24s they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of mimicoin24s being spent is exactly
...
- **Increased security for multisig:** Mimicoin24 addresses (both P2PKH addresses
  ...
  believed to be provided by Mimicoin24's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Mimicoin24's current ledger (called the UTXO set),
  ...
  note that Mimicoin24 Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Mimicoin24 users to individually opt-in to almost any change in the Mimicoin24
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Mimicoin24 Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Mimicoin24
client to Mimicoin24 Core 0.13.2 to signal for segwit support. When a
...
Since Mimicoin24 Core 0.10.0, nodes have defaulted to only relaying and
...
- `mimicoin24-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `mimicoin24-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
