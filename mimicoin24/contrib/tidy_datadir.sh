...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Mimicoin24 database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Mimicoin24 datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Mimicoin24 datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Mimicoin24 0.7 datadir."
    ;;
  3)
    echo "Detected Mimicoin24 pre-0.8 datadir."
    ;;
  4)
    echo "Detected Mimicoin24 0.8 datadir."
    ;;
esac
